...
class SlowFingerprints(Fingerprints):
    ...
    def id_count_tanimoto_hits(self, queries, threshold=0.7, batch_size=100):
        _check_threshold(threshold)
        for query_arena in queries.iter_arenas(batch_size):
            for result in count_tanimoto_hits(query_arena, self._id_fp_pairs, threshold):
                yield result

    def id_threshold_tanimoto_search_fp(self, fp, threshold=0.7):
        _check_threshold(threshold)
        return threshold_tanimoto_search_fp(fp, self._id_fp_pairs, threshold)

    def id_threshold_tanimoto_search(self, queries, threshold=0.7, batch_size=100):
        _check_threshold(threshold)

        for query_arena in queries.iter_arenas(batch_size):
            for result in threshold_tanimoto_search(query_arena, self._id_fp_pairs,
                                                    threshold):                                                    
                yield result

    def id_knearest_tanimoto_search_fp(self, fp, k=3, threshold=0.7):
        if k < 0:
            raise ValueError("k must be non-negative")
        _check_threshold(threshold)
        return knearest_tanimoto_search_fp(fp, self._id_fp_pairs, k, threshold)

    def id_knearest_tanimoto_search(self, queries, k=3, threshold=0.7, batch_size=100):
        if k < 0:
            raise ValueError("k must be non-negative")
        _check_threshold(threshold)
        
        for query_arena in queries.iter_arenas(batch_size):
            for result in knearest_tanimoto_search(query_arena, self._id_fp_pairs,
                                                   k, threshold):
                yield result
